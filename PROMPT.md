...
- 已知约束（规范/兼容性/性能/安全）：
  ...
  - 微信鉴权链路已支持真实配置（secrets/env），但真实 code 联调待小程序端提供。
  - 角色权限通过种子落库，策略由 API Host 注册。

## 3) 当前任务（本次要继续的核心）
- 任务标题：继续推进后端最小业务闭环（Customer 侧“我的订单”）。
- 期望结果（Definition of Done）：
  - 提供 Customer 角色可访问的“我的订单”最小 API（查询列表/详情其一或两者）。
  - 接口接入 Customer 权限控制并可通过鉴权验证。
  - 与已落地 Phase 2 订单模型保持一致。
- 非目标（明确这次不做什么）：
  - 不在本步骤切换微信真实凭证。
  - 不在本步骤扩展复杂订单状态流转与退款流程。
...
- [x] 已完成 6：微信配置切换到真实模式（默认关闭 Mock）+ 启动校验 + secrets/env 注入方案。
- [x] 已完成 7：Phase 2 核心表建模与迁移（ServiceItem/CapacitySchedule/Order/CouponTemplate/UserCoupon）。
- [x] 已完成 8：`Order.SnapshotData` 映射、`CapacitySchedule.RowVersion` 并发字段与模型级约束测试。
- [ ] 未完成 1：真实 `code` 的微信端到端联调（等待小程序端开工）。
- [ ] 未完成 2：Customer 最小业务闭环 API（“我的订单”）。
...
- 决策 C：
  - 方案：运行配置默认关闭 Mock，凭据通过 secrets/env 注入。
  - 原因：避免误用 Mock 配置进入真实环境，缺失凭据时启动即失败（Fail Fast）。
...
- 关键配置项：
  ...
  - WeChat:MiniApp:EnableMockMode=false
- 依赖版本（非常重要）：
  - .NET SDK 9.x（编译），目标框架 net8.0
  - ABP 8.3.4
  - OpenIddict（ABP 集成）
- 外部限制（API 配额、权限、网络）：
  - 真实微信联调需要小程序端可提供有效 code。
  - 真实微信接口要求外网可达 api.weixin.qq.com 且具备合法 AppId/AppSecret。
...
- src/aspnet-core/src/CY.HomeCleaning.Domain/Business/：Phase 2 核心实体（ServiceItem/CapacitySchedule/Order/CouponTemplate/UserCoupon）。
- src/aspnet-core/src/CY.HomeCleaning.Domain.Shared/Business/：订单/计费/券状态枚举定义。
- src/aspnet-core/src/CY.HomeCleaning.EntityFrameworkCore/Migrations/20260408134743_AddCoreBusinessTables_Phase2.cs：Phase 2 表结构迁移。
...
## 8) 待办清单（按优先级）
1. P0：基于 Customer 角色新增“我的订单”等最小业务闭环 API，并接入权限控制。
2. P1：补充订单创建后占用运力的防超卖业务级测试。
3. P2：待小程序端具备条件后，完成微信真实 code 联调并回填验证记录。
...
## 10) 下一步指令（每次续聊只改这里）
请基于上述上下文，继续完成：  
“实现 Customer 端‘我的订单’最小可用 API（优先列表查询），接入 CustomerOnly 权限策略，提供最小 DTO 与查询逻辑，并补充至少 1 条可运行测试。”